...
struct TasksView: View {
    ...
    var onPlanDay: () -> Void
    var onStartDay: () -> Void  // Add this line

    var body: some View {
        HStack(spacing: 20) {
            TaskListView(categoryManager: categoryManager, dailyFocusManager: dailyFocusManager)
        ...
        .sheet(isPresented: $showDailyPlan) {
            DailyPlanView(categoryManager: categoryManager, dailyFocusManager: dailyFocusManager, onDismiss: {
                showDailyPlan = false
            }, onStartDay: onStartDay)  // Pass the onStartDay closure here
        }
    }
    ...
    private var planYourDayButton: some View {
        Button(action: {
            withAnimation(.spring(response: 0.4, dampingFraction: 0.75)) {
                onPlanDay()  // Use the onPlanDay closure instead of showing a sheet
            }
        }) {
            Text("Plan Your Day")
