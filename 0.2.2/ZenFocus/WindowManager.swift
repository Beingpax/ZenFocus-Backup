...
class WindowManager: ObservableObject {
    ...
    func showFocusedTaskWindow(for task: ZenFocusTask, dailyFocusManager: DailyFocusManager, onComplete: @escaping (ZenFocusTask) -> Void, onBreak: @escaping () -> Void) {
        DispatchQueue.main.async {
            self.hideMainWindowWithAnimation()
            self.playFocusSound()
            self.showFullScreenAnimation(for: task) {
                self.removeAnimationWindow() // Ensure animation window is removed
                self.createAndShowFocusedTaskWindow(for: task, dailyFocusManager: dailyFocusManager, onComplete: onComplete, onBreak: onBreak)
            }
    ...
    private func createAndShowFocusedTaskWindow(for task: ZenFocusTask, dailyFocusManager: DailyFocusManager, onComplete: @escaping (ZenFocusTask) -> Void, onBreak: @escaping () -> Void) {
        guard let screen = NSScreen.main else {
            ZenFocusLogger.shared.error("No main screen found")
            return
        }
        
        let windowController = FocusSessionWindowController(task: task, windowManager: self, dailyFocusManager: dailyFocusManager)
        let focusedView = FocusSessionView(task: task, dailyFocusManager: dailyFocusManager, windowController: windowController)
